...
(function () {
  ...
  var DICT = {
    ...
    'zh-TW': {
      ...
      'pv.s3t': '網路與權限',
      'pv.s3b': '相機僅在你點擊掃碼或拍照辨識時申請，用於掃描 SKU 二維碼或拍攝生豆袋標籤，OCR 在裝置端完成；拒絕後可手動輸入、搜尋或從系統圖片選擇器匯入。排程提醒使用本機通知，不讀取或寫入系統日曆。通知僅在你主動開啟庫存預警或排程提醒時申請；拒絕後仍顯示 App 內橫幅。系統檔案選擇器只讀取你明確選擇的圖片、檔案或共享目錄。',
      'pv.s4t': '離線 OCR 與圖片',
    ...
    'en': {
      ...
      'pv.s3b': 'Camera access is requested only when you tap scan or photograph-recognition, to scan SKU QR codes or photograph green-bean labels; OCR runs on-device. If you refuse, you can type, search, or choose an image manually. Production reminders use local notifications and do not read or write the system calendar. Notifications are requested only when you actively enable stock or production reminders; refusal leaves in-app banners available. The system file picker reads only files or directories that you explicitly choose.',
