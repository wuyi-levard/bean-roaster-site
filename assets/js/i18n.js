...
(function () {
  ...
  var DICT = {
    ...
    'zh-TW': {
      ...
      'pv.eyebrow': '隱私政策',
      'pv.h1': '隱私政策',
      'pv.version': '2026-09-02.v1',
      'pv.updated': '政策版本：2026-09-02.v1 · 生效日期：2026 年 9 月 2 日 · 個人資料處理者：林雲',
      ...
      'pv.s3t': '網路與權限',
      'pv.s3b': '相機僅在你點擊掃碼或拍照辨識時申請，用於掃描 SKU 二維碼或拍攝生豆袋標籤，OCR 在裝置端完成；拒絕後可手動輸入、搜尋或從系統圖片選擇器匯入。日曆讀寫僅在你點擊同步排程提醒時申請；拒絕後仍可使用排程、訂單和庫存。通知僅在你主動開啟庫存預警時申請；拒絕後仍顯示 App 內橫幅。系統檔案選擇器只讀取你明確選擇的圖片、檔案或共享目錄。',
      'pv.s4t': '離線 OCR 與圖片',
      ...
      'pv.s7t': '日誌與診斷',
      'pv.s7b': 'App 僅在本機保留最近 50 條錯誤日誌，用於你自行排查，可手動匯出；預設不會發送給我們。官網由騰訊雲輕量應用伺服器託管；正式站點啟用後，Web 服務可能記錄 IP、請求時間、請求路徑、回應狀態和 User-Agent 等必要訪問日誌，僅用於安全、防攻擊和故障排查，完成日誌輪轉配置後按 7 天保留並自動刪除。備案攔截期間請求不會到達源站，源站沒有對應存取日誌屬於正常現象。',
      'pv.s8t': '本網站（介紹站）',
    ...
    'en': {
      ...
      'pv.version': '2026-09-02.v1',
      'pv.updated': 'Policy version: 2026-09-02.v1 · Effective date: September 2, 2026 · Personal information processor: Yun Lin',
      ...
      'pv.s3b': 'Camera access is requested only when you tap scan or photograph-recognition, to scan SKU QR codes or photograph green-bean labels; OCR runs on-device. If you refuse, you can type, search, or choose an image manually. Calendar read/write is requested only when you tap sync for a production reminder; refusal does not block production plans, orders, or inventory. Notifications are requested only when you actively enable stock alerts; refusal leaves in-app banners available. The system file picker reads only files or directories that you explicitly choose.',
      ...
      'pv.s7b': 'The app keeps the last 50 error logs locally for your troubleshooting and can export them manually; they are not sent to us by default. The website is hosted on a Tencent Cloud Light Application Server. Once the production site is enabled, its web service may record IP address, request time, path, status, and User-Agent for security and fault diagnosis only; after log rotation is configured, logs are retained for 7 days and then deleted. During filing-blocking, requests do not reach the origin server, so missing origin access logs are expected.',
      'pv.s8t': 'This website',
